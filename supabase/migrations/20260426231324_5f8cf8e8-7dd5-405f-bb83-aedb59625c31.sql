-- 1) Ajouter la colonne sizes (tableau) sur product_variants
ALTER TABLE public.product_variants
  ADD COLUMN IF NOT EXISTS sizes public.product_size[] NOT NULL DEFAULT ARRAY[]::public.product_size[];

-- 2) Migrer les variantes existantes : si "size" est défini, le mettre dans "sizes"
UPDATE public.product_variants
   SET sizes = ARRAY[size]::public.product_size[]
 WHERE size IS NOT NULL
   AND (sizes IS NULL OR array_length(sizes, 1) IS NULL);

-- 3) Mettre à jour la fonction place_order pour utiliser variant_label = name + color (sans size)
--    et conserver ci.size comme la taille choisie par le client.
CREATE OR REPLACE FUNCTION public.place_order(
  p_reference text, p_customer_name text, p_customer_phone text,
  p_customer_address text, p_notes text, p_payment_operator text,
  p_payment_reference text, p_confirmed_shop_ids uuid[]
)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
